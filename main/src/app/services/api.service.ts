...
export class ApiService {
  ...
  public static apiDialogFlow = ApiService.apiVersion + environment.dialogFlow;
  ...
  get(url: string): Observable<any> {
    ...
    return this.http.get<any>(environment.apiUrl + url).pipe(map((res: any) => res),
  ...
  put(url: string, entities: string, id: number): Observable<any> {
    ...
    return this.http.put(environment.apiUrl + url + id, entities).pipe(
      map((res: any) => res,
  ...
  patch(url: string, entities: any): Observable<any> {
    return this.http.patch(url, entities).pipe(map((res: any) => res,
      catchError(err => throwError(err)
      )));
  }
  ...
  delete(url: string, id: number): Observable<any> {
    return this.http.delete(`${environment.apiUrl}${url}/${id}`).pipe(map((res: any) => res,
      catchError(err => throwError(err)
      )));
  }
  ...
  post(url: string, entity: any): Observable<any> {
    ...
    return this.http.post(`${environment.apiUrl}${url}`, entity).pipe(map((res: any) => res,
      catchError((err: any) => {
          console.log(err);
